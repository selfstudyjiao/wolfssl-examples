...
typedef struct ms_ecall_output_secret_addr_t {
	uint64_t* ms_secret_addr;
} ms_ecall_output_secret_addr_t;
...
static sgx_status_t SGX_CDECL sgx_ecall_output_secret_addr(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_output_secret_addr_t));
	ms_ecall_output_secret_addr_t* ms = SGX_CAST(ms_ecall_output_secret_addr_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint64_t* _tmp_secret_addr = ms->ms_secret_addr;
	size_t _len_secret_addr = sizeof(*_tmp_secret_addr);
	uint64_t* _in_secret_addr = NULL;

	CHECK_UNIQUE_POINTER(_tmp_secret_addr, _len_secret_addr);

	if (_tmp_secret_addr != NULL && _len_secret_addr != 0) {
		if ((_in_secret_addr = (uint64_t*)malloc(_len_secret_addr)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_secret_addr, 0, _len_secret_addr);
	}
	ecall_output_secret_addr(_in_secret_addr);
err:
	if (_in_secret_addr) {
		memcpy(_tmp_secret_addr, _in_secret_addr, _len_secret_addr);
		free(_in_secret_addr);
	}
...
SGX_EXTERNC const struct {
	size_t nr_ecall;
	struct {void* ecall_addr; uint8_t is_priv;} ecall_table[23];
} g_ecall_table = {
	23,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_output_secret_addr, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[5][23];
} g_dyn_entry_table = {
	5,
	{
